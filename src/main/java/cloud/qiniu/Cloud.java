...
public class Cloud {
    ...
    //鉴定是否涉黄
    public boolean isEroticism(String picUrl){

//        net.dongliu.requests.Response<String> response = Requests.get(picUrl).verify(false).text("UTF-8");
//        System.out.println(response.getBody());
        ...
        try {
            ...
            String resp = Requests.get(picUrl).send().readToText(StandardCharsets.UTF_8);

            System.out.println(resp);

            Map<String, Object> resultMap = JSON.parseObject(resp);
            int code = (int) resultMap.get("code");
            if (code == 0) {
                JSONArray fileListArray = (JSONArray) resultMap.get("fileList");
                Map<String, Object> fileMap = fileListArray.getJSONObject(0);
                int label = (int) fileMap.get("label");
                if (label == 0) {
                    return true;
                }
     ...
     * */
    public String simpleUploadWithoutKey(byte[] file) throws QiniuException {
        Auth auth = Auth.create(this.accessKey,
                this.secretKey);
        String token = auth.uploadToken(this.publicBucket);
        UploadManager um = new UploadManager();
        ...
        try {
            Response resp = um.put(file, null,
                    token);
            Map<String,Object> map = JSON.parseObject(resp.bodyString());
